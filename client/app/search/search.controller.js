 ...
 $scope.goSearch = function(){
     ...
     if (typeof $scope.search !== 'undefined')
     {
       if ($scope.search.date)
       {
           ...
           {
               $scope.search.$and=[ { timeStamp: { $gte: $scope.search.date.from } }, { timeStamp: { $lte: $scope.search.date.to } } ];
           }
           else if($scope.search.date.from){
               $scope.search.timeStamp = {$gte: $scope.search.date.from};
           }
            else if($scope.search.date.to){
               $scope.search.timeStamp = {$lte: $scope.search.date.to};
           }
